...
export default async function AziPage({ searchParams }: { searchParams: Promise<{ lat?: string; lng?: string; loc?: string }> }) {
    ...
    return (
        <div className="pb-20 pt-4 md:pt-8 relative">
            ...
            <div className="container-custom px-4 relative z-10">
                {/* Hero */}
                <div className="mb-8 md:mb-10 card-panel taste-surface p-5 md:p-8">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <h1 className="text-4xl md:text-6xl font-display font-bold tracking-tight leading-none text-white mb-4">
                                Solunar <span className="text-amber-400">Azi</span>
                            </h1>
                            <p className="text-slate-400 text-sm md:text-base mb-2">{capitalizedDate} &bull; {locationName}</p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
                    ...
                    <div className="card-glass p-4 text-center">
                        <p className="text-[10px] uppercase text-slate-400 font-bold tracking-wider mb-1">Apus Soare</p>
                        <p className="text-xl font-mono text-amber-300">{formatTime(data.sunset)}</p>
                    </div>
                    <div className="card-glass p-4 text-center">
                        <p className="text-[10px] uppercase text-slate-400 font-bold tracking-wider mb-1">Răsărit Lună</p>
                        <p className="text-xl font-mono text-white">{data.moonrise ? formatTime(data.moonrise) : '--:--'}</p>
                    </div>
                    <div className="card-glass p-4 text-center">
                        <p className="text-[10px] uppercase text-slate-400 font-bold tracking-wider mb-1">Apus Lună</p>
                        <p className="text-xl font-mono text-white">{data.moonset ? formatTime(data.moonset) : '--:--'}</p>
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                    <div className="card-panel taste-surface p-6">
                        <h2 className="text-xl font-display font-bold text-amber-200 mb-4">Specii Active Azi</h2>
                        {activeFish.length > 0 ? (
                            <div className="flex flex-wrap gap-2">
                                {activeFish.map((fish, i) => (
                                    <span key={i} className="px-3 py-1.5 bg-amber-300/10 border border-amber-200/20 rounded-full text-amber-100 text-sm font-medium">
                                        {fish}
                                    </span>
                                ))}
                            </div>
                        ) : (
                            <p className="text-slate-400">Activitate generală redusă astăzi.</p>
                        )}
                    </div>
                    <div className="card-panel taste-surface p-6">
                        <h2 className="text-xl font-display font-bold text-white mb-4">Sfatul Zilei</h2>
                ...
                <div className="card-panel p-6 md:p-8 mb-8">
                    ...
                    <p className="text-slate-400 text-sm mb-4">Află când trage fiecare specie, ce momeli să folosești și care sunt perioadele solunar optime:</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {[
                            { name: 'Crap', slug: 'pescuit-crap' },
                            { name: 'Șalău', slug: 'pescuit-salau' },
                            { name: 'Somn', slug: 'pescuit-somn' },
                            { name: 'Știucă', slug: 'pescuit-stiuca' },
                            { name: 'Păstrăv', slug: 'pescuit-pastrav' },
                            { name: 'Caras', slug: 'pescuit-caras' },
                            { name: 'Biban', slug: 'pescuit-biban' },
                            { name: 'Plătică', slug: 'pescuit-platica' },
                        ].map(fish => (
                            <Link key={fish.slug} href={`/${fish.slug}`} className="p-3 bg-white/[0.045] hover:bg-white/[0.075] border border-white/10 rounded-xl transition-colors text-center">
                                <p className="text-white font-bold text-sm">{fish.name}</p>
                            </Link>
                        ))}
                    </div>
                </div>
                ...
                <div className="card-panel p-6 md:p-8">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <Link href="/" className="p-4 bg-white/5 hover:bg-white/10 rounded-xl text-center transition-colors">
                            ...
                            <p className="text-slate-400 text-xs mt-1">Prognoză 14 zile</p>
                        </Link>
                        <Link href="/lunar" className="p-4 bg-white/5 hover:bg-white/10 rounded-xl text-center transition-colors">
                            ...
                            <p className="text-slate-400 text-xs mt-1">Calendar lunar 2026</p>
                        </Link>
                        <Link href="/blog" className="p-4 bg-white/5 hover:bg-white/10 rounded-xl text-center transition-colors">
                            ...
                            <p className="text-slate-400 text-xs mt-1">Articole și sfaturi</p>
                        </Link>
                        <Link href="/blog/cum-functioneaza-calendarul-solunar" className="p-4 bg-white/5 hover:bg-white/10 rounded-xl text-center transition-colors">
                            ...
                            <p className="text-slate-400 text-xs mt-1">Teoria solunar</p>
                        </Link>
                    </div>
                </div>
